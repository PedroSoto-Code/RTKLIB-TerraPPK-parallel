...
/* Contexto de procesamiento para un hilo */
typedef struct {
...
/* global variables for combined mode */
static sol_t *solf=NULL;        /* forward solutions */
static sol_t *solb=NULL;        /* backward solutions */
static double *rbf=NULL;        /* forward base positions */
static double *rbb=NULL;        /* backward base positions */
static int isolf=0,isolb=0;     /* solution indices */
static int iobsu=0,iobsr=0;     /* observation indices (legacy) */
static int isbs=0;              /* sbas index (legacy) */
static int reverse=0;           /* reverse mode (legacy) */
static int aborts=0;            /* abort status (legacy) */
...
{
    trace(3,"\ninfunc  : dir=%d iobsu=%d iobsr=%d isbs=%d\n",ctx->reverse,ctx->iobsu,ctx->iobsr,ctx->isbs);

    if (0<=ctx->iobsu&&ctx->iobsu<obss.n) {
        gtime_t time = obss.data[ctx->iobsu].time;
        settime(time);
        char tstr[40];
    ...
    if (!ctx->reverse) {
        /* Input forward data */
        int nu=nextobsf(&obss,&ctx->iobsu,1);
        if (nu<=0) return -1;
        for (int i=0;i<nu&&n<MAXOBS*2;i++) obs[n++]=obss.data[ctx->iobsu+i];
        if (ctx->iobsr<obss.n) {
            if (popt->intpref) {
                /* For interpolation, find first base timestamp after rover timestamp */
                int nr=nextobsf(&obss,&ctx->iobsr,2);
                while (nr>0) {
                    if (timediff(obss.data[ctx->iobsr].time,obss.data[ctx->iobsu].time)>-DTTOL) break;
                    ctx->iobsr+=nr;
                    nr=nextobsf(&obss,&ctx->iobsr,2);
                }
            } else {
                /* If not interpolating, find the closest iobsr timestamp before or after iobsu. */
                double dt=fabs(timediff(obss.data[ctx->iobsr].time,obss.data[ctx->iobsu].time));
                int i=ctx->iobsr,nr=nextobsf(&obss,&i,2);
                while (nr>0) {
                    double dt_next=fabs(timediff(obss.data[i].time,obss.data[ctx->iobsu].time));
                    if (dt_next>dt) break;
                    dt=dt_next;
                    ctx->iobsr=i;
                    i+=nr;
            ...
            int nr=nextobsf(&obss,&ctx->iobsr,2);
            for (int i=0;i<nr&&n<MAXOBS*2;i++) obs[n++]=obss.data[ctx->iobsr+i];
        }
        ctx->iobsu+=nu;

        /* Update sbas corrections */
        while (ctx->isbs<sbss.n) {
            gtime_t time=gpst2time(sbss.msgs[ctx->isbs].week,sbss.msgs[ctx->isbs].tow);

            if (getbitu(sbss.msgs[ctx->isbs].msg,8,6)!=9) { /* Except for geo nav */
                sbsupdatecorr(sbss.msgs+ctx->isbs,&navs);
            }
        ...
        /* Input backward data */
        int nu=nextobsb(&obss,&ctx->iobsu,1);
        if (nu<=0) return -1;
        for (int i=0;i<nu&&n<MAXOBS*2;i++) obs[n++]=obss.data[ctx->iobsu-nu+1+i];
        if (ctx->iobsr>=0) {
            if (popt->intpref) {
                /* For interpolation, find first base timestamp before rover timestamp */
                int nr=nextobsb(&obss,&ctx->iobsr,2);
                while (nr>0) {
                  if (timediff(obss.data[ctx->iobsr].time,obss.data[ctx->iobsu].time)<DTTOL) break;
                  ctx->iobsr-=nr;
                  nr=nextobsb(&obss,&ctx->iobsr,2);
                }
            } else {
                /* If not interpolating, find the closest iobsr timestamp before or after iobsu. */
                double dt=fabs(timediff(obss.data[ctx->iobsr].time,obss.data[ctx->iobsu].time));
                int i=ctx->iobsr,nr=nextobsb(&obss,&i,2);
                while (nr>0) {
                    double dt_next=fabs(timediff(obss.data[i].time,obss.data[ctx->iobsu].time));
                    if (dt_next>dt) break;
                    dt=dt_next;
            ...
            int nr=nextobsb(&obss,&ctx->iobsr,2);
            for (int i=0;i<nr&&n<MAXOBS*2;i++) obs[n++]=obss.data[ctx->iobsr-nr+1+i];
        }
        ctx->iobsu-=nu;
...
static void procpos(FILE *fp, FILE *fptm, proc_ctx_t *ctx, int mode)
{
    ...
    solstatic=ctx->sopt.solstatic&&
              (ctx->popt.mode==PMODE_STATIC||ctx->popt.mode==PMODE_STATIC_START||ctx->popt.mode==PMODE_PPP_STATIC);
    
    rtcm_path[0]='\0';

    while ((nobs=inputobs(obs_ptr,ctx->rtk.sol.stat,&ctx->popt,ctx))>=0) {

        /* exclude satellites */
        for (i=n=0;i<nobs;i++) {
            if ((satsys(obs_ptr[i].sat,NULL)&ctx->popt.navsys)&&
                ctx->popt.exsats[obs_ptr[i].sat-1]!=1) obs_ptr[n++]= obs_ptr[i];
        }
        if (n<=0) continue;

        /* carrier-phase bias correction */
        if (!strstr(ctx->popt.pppopt,"-ENA_FCB")) {
            corr_phase_bias_ssr(obs_ptr,n,&navs);
        }
        if (!rtkpos(&ctx->rtk, obs_ptr,n,&navs)) {
            if (ctx->rtk.sol.eventime.time != 0) {
                if (mode == SOLMODE_SINGLE_DIR) {
                    outinvalidtm(fptm, &ctx->sopt, ctx->rtk.sol.eventime);
                } else if (!ctx->reverse&&ctx->nitm<MAXINVALIDTM) {
                    ctx->invalidtm[ctx->nitm++] = ctx->rtk.sol.eventime;
                }
            }
            continue;
        }
        ...
        if (mode==SOLMODE_SINGLE_DIR) {    /* forward or backward */
            if (!solstatic) {
                outsol(fp,&ctx->rtk.sol,ctx->rtk.rb,&ctx->sopt);
            }
            else if (time.time==0||pri[ctx->rtk.sol.stat]<=pri[sol.stat]) {
                sol=ctx->rtk.sol;
                for (i=0;i<3;i++) rb[i]=ctx->rtk.rb[i];
                if (time.time==0||timediff(ctx->rtk.sol.time,time)<0.0) {
                    time=ctx->rtk.sol.time;
                }
            }
            /* check time mark */
            if (ctx->rtk.sol.eventime.time != 0)
            {
                newsol = fillsoltm(oldsol,ctx->rtk.sol,ctx->rtk.sol.eventime);
                num++;
                if (!solstatic&&mode==SOLMODE_SINGLE_DIR) {
                    outsol(fptm,&newsol,rb,&ctx->sopt);
                }
            }
            oldsol = ctx->rtk.sol;
        }
            ...
            ctx->sol[ctx->isol]=ctx->rtk.sol;
            for (i=0;i<3;i++) ctx->rb[i+ctx->isol*3]=ctx->rtk.rb[i];
            ctx->isol++;
        }
    }
    if (mode==SOLMODE_SINGLE_DIR && solstatic&&time.time!=0.0) {
        sol.time=time;
        outsol(fp,&sol,rb,&ctx->sopt);
    }
...
static void combres(FILE *fp, FILE *fptm, const prcopt_t *popt, const solopt_t *sopt)
{
    ...
    int iitm=0;  /* invalid time mark index */
...
static DWORD WINAPI thread_proc(LPVOID arg) {
    ...
    procpos(NULL, NULL, ctx, SOLMODE_COMBINED);
    return 0;
}
#else
    ...
    if (popt_.mode==PMODE_SINGLE||popt_.soltype==SOLTYPE_FORWARD) {
        ...
        if (fp) {
            ...
            if (fptm) {
                ...
                ctx->iobsu=iobsu; ctx->iobsr=iobsr; ctx->isbs=isbs; ctx->reverse=reverse;
                ctx->popt=popt_; ctx->sopt=*sopt;
                rtkinit(&ctx->rtk,&popt_);
                procpos(fp,fptm,ctx,SOLMODE_SINGLE_DIR);
                rtkfree(&ctx->rtk);
                free(ctx);
                fclose(fptm);
            }
        ...
        if (solf&&solb) {
            ...
            if (popt_.soltype == SOLTYPE_COMBINED) {
                ...
                ctx_f->iobsu = 0; ctx_f->iobsr = 0; ctx_f->isbs = 0; ctx_f->reverse = 0;
                ctx_f->isol = 0; ctx_f->sol = solf; ctx_f->rb = rbf;
                ctx_f->popt = popt_; ctx_f->sopt = *sopt;
                rtkinit(&ctx_f->rtk, &popt_);
                
                /* Init Backward Context */
                ctx_b->iobsu = obss.n - 1; ctx_b->iobsr = obss.n - 1; ctx_b->isbs = sbss.n - 1; ctx_b->reverse = 1;
                ctx_b->isol = 0; ctx_b->sol = solb; ctx_b->rb = rbb;
                ctx_b->popt = popt_; ctx_b->sopt = *sopt;
                rtkinit(&ctx_b->rtk, &popt_);
                ...
                isolf = ctx_f->isol;
                isolb = ctx_b->isol;
                aborts = ctx_f->aborts | ctx_b->aborts;
                
                free(ctx_f);
                free(ctx_b);
                
            } else {
                /* Serial execution (original logic but using contexts) */
                proc_ctx_t *ctx = (proc_ctx_t *)calloc(1, sizeof(proc_ctx_t));
                ctx->iobsu=0; ctx->iobsr=0; ctx->isbs=0; ctx->reverse=0;
                ctx->isol=0; ctx->sol=solf; ctx->rb=rbf;
                ctx->popt=popt_; ctx->sopt=*sopt;
                rtkinit(&ctx->rtk,&popt_);
                
                procpos(NULL,NULL,ctx,SOLMODE_COMBINED); /* forward */
                isolf = ctx->isol;
                
                /* Reset for backward */
                ctx->reverse=1; ctx->iobsu=obss.n-1; ctx->iobsr=obss.n-1; ctx->isbs=sbss.n-1;
                ctx->isol=0; ctx->sol=solb; ctx->rb=rbb;
                ...
                procpos(NULL,NULL,ctx,SOLMODE_COMBINED); /* backward */
                isolb = ctx->isol;
                aborts = ctx->aborts;
